...
character = function(){
	...
	this.player={}
	for (var i = 0; i < 3; i++){
		this.player[i]=game.add.sprite(game.world.centerX,window.innerHeight+100,'particle_character')	
...
character.prototype.show_star = function() {
	...
	this.tween5.onComplete.add(function(){game.time.events.add(100,this.audio_star,this)})
}
			...
			for (var i = 0; i < 3; i++){
				for (var j = 0; j < canon.length; j++){
					if(canon[j].special_color){
						...
						game.physics.arcade.collide(canon[j].weapon.bullets,hero.player[i],function(){hero.explode(i)})
					}
				}
			}
		}
